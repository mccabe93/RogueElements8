...
namespace RogueElements.Examples.Ex5_Terrain
{
    public static class Example5
    {
        public static void Run()
        {
            ...
            {
                RoomRatio = new RandRange(25),
                BranchRatio = new RandRange(0, 10),
            };

            var genericRooms = new SpawnList<RoomGen<MapGenContext>>
            {
                { new RoomGenRound<MapGenContext>(new RandRange(3, 3), new RandRange(3, 3)), 10 }, // round
            };
            ...
            const int terrain = 2;
            var waterPostProc = new PerlinWaterStep<MapGenContext>(new RandRange(75), 3, new Tile(terrain), new MapTerrainStencil<MapGenContext>(false, true, false), 1);
